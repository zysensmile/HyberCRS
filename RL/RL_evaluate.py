...
def dqn_evaluate(args, kg, dataset, agent, filename, i_episode):
    test_env = EnvDict[args.data_name](kg, dataset, args.data_name, args.embed, seed=args.seed, max_turn=args.max_turn,
                                       ...
                                       mode='test2', ask_num=args.ask_num, entropy_way=args.entropy_method,
    ...
    for user_num in tqdm(range(user_size)):  # user_size
        ...
        enablePrint()
    ####
    rec_times = test_env.rec_times
    # rec_item_user_a = test_env.rec_item_user_a
    # rec_item_user_b = test_env.rec_item_user_b
    items_set = set(test_env.rec_item_user_a.keys()) | set(test_env.rec_item_user_b.keys())
    item_coverage = len(items_set) / test_env.item_length / rec_times
    # isolation = 0
    # a_n = 0
    # b_n = 0
    # for item in items_set:
    #     if item not in rec_item_user_a.keys():
    #         rec_item_user_a[item] = 0
    #     if item not in rec_item_user_b.keys():
    #         rec_item_user_b[item] = 0
    #     a_i = rec_item_user_a[item]
    #     b_i = rec_item_user_b[item]
    #     a_n += a_i
    #     b_n += b_i
    # for item in items_set:
    #     a_i = rec_item_user_a[item]
    #     b_i = rec_item_user_b[item]
    #     w = a_i / (a_i + b_i)
    #     isolation += w * (a_i / a_n - b_i / b_n)
    print('item_coverage:', item_coverage)

    ask_times = test_env.ask_times
    atts_set = set(test_env.ask_att_user_a.keys()) | set(test_env.ask_att_user_b.keys())
    att_coverage = len(atts_set) / test_env.feature_length / ask_times
    print('att_coverage:', att_coverage)
