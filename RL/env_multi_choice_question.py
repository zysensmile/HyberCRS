...
class MultiChoiceRecommendEnv(object):
    def __init__(self, kg, dataset, data_name, embed, seed=1, max_turn=15, cand_num=10, cand_item_num=10, attr_num=20, mode='train', ask_num=1, entropy_way='weight entropy', fm_epoch=0,choice_num=4,fea_score="weight entropy"):
        ...
        self.cover_num = 500
        self.rec_times = 0
        self.rec_item_user_a = {}
        self.rec_item_user_b = {}
        self.ask_times = 0
        self.ask_att_user_a = {}
        self.ask_att_user_b = {}
    ...
    def __test_tuple_generate__(self):
        ...
        """
    def __test_tuple_generate2__(self):####
        start = 800
        ui_list = []
        for user_str in self.u_multi:
            user_id = int(user_str)
            for pair in self.u_multi[user_str]:
                ui_list.append([user_id, pair])
                break
        self.ui_array = np.array(ui_list[start:start+self.cover_num], dtype = object)
        # np.random.shuffle(self.ui_array)
    ...
    def _get_state(self, cand_act):
        ...
        friend_item_dict = {}#friend_item[friend] = item_list
        for friend in self.friends_ids:
            item_list = [item for item in self.kg.G['user'][friend]['interact'] if item in set_cand_items]
            if len(item_list)>0:
                friend_item_dict[friend] = item_list
        self.friends_ids = list(friend_item_dict.keys())
        self.friend_to_item = friend_item_dict
        ...
        for friend in self.friends_ids:
            feature_list = []
            for item in friend_item_dict[friend]:
                feature_list += [feature for feature in self.kg.G['item'][item]['belong_to'] if
                                   feature in set_reachable_feature]
            friend_feature_dict[friend] = list(set(feature_list))
            for fea in feature_list:
                if fea not in feature_count_dict.keys():
                    feature_count_dict[fea] = 0
                feature_count_dict[fea] += 1

            for fea in list(set(feature_list)):
                if fea not in feature_friend_dict.keys():
                    feature_friend_dict[fea] = []
                feature_friend_dict[fea].append(friend)
        feature_count_list = sorted(feature_count_dict.items(), key=lambda x: x[1], reverse=True)[:topk]
        feature_friend = [x for (x, y) in feature_count_list]
        ...
        index = []
        value = []
        #like view #feature hypergraph
        lens.append(len(self.user_acc_feature))
        for fea in self.user_acc_feature:
            fea_id = fea + self.user_length + self.item_length
            user_idx, idx_v = self.get_key(idx_v_dict, self.user_id, idx_v)
            index.append([user_idx, idx_e])
            value.append(1)
            feature_idx, idx_v = self.get_key(idx_v_dict, fea_id, idx_v)
            index.append([feature_idx, idx_e])
            value.append(1)
            if fea in feature_item_pair.keys():
                for item in feature_item_pair[fea]:
                    item_id = item + self.user_length
                    item_idx, idx_v = self.get_key(idx_v_dict, item_id, idx_v)
                    index.append([item_idx, idx_e])
                    value.append(1 / len(feature_item_pair[fea]))
            idx_e += 1
        #dislike view #feature hypergraph
        lens.append(len(self.user_rej_feature))
        for fea in self.user_rej_feature:
            ...
            index.append([feature_idx, idx_e])
            value.append(1)
            if fea in feature_neg_item_pair.keys():
                for item in feature_neg_item_pair[fea]:
                    item_id = item + self.user_length
                    item_idx, idx_v = self.get_key(idx_v_dict, item_id, idx_v)
                    index.append([item_idx, idx_e])
                    value.append(1 / len(feature_neg_item_pair[fea]))
            idx_e += 1
        #social view #feature hypergraph
        lens.append(len(feature_friend))
        for fea in feature_friend:
            fea_id = fea + self.user_length + self.item_length
            feature_idx, idx_v = self.get_key(idx_v_dict, fea_id, idx_v)
            index.append([feature_idx, idx_e])
            value.append(1)
            if fea in feature_item_pair.keys():
                for item in feature_item_pair[fea]:
                    ...
                    index.append([item_idx, idx_e])
                    value.append(1 / len(feature_item_pair[fea]))
            for friend in feature_friend_dict[fea]:
                friend_idx, idx_v = self.get_key(idx_v_dict, friend, idx_v)
                index.append([friend_idx, idx_e])
                value.append(1 / len(feature_friend_dict[fea]))
            idx_e += 1
        ...
        for friend in self.friends_ids:
            for item in friend_item_dict[friend]:
                if item not in item_friend_dict.keys():
                    item_friend_dict[item] = []
                if item not in item_count_dict.keys():
                    item_count_dict[item] = 0
                item_count_dict[item] += 1
                item_friend_dict[item].append(friend)
        item_count_list = sorted(item_count_dict.items(), key=lambda x: x[1], reverse=True)[:topk]
        item_friend = [x for (x, y) in item_count_list]

        history_items = [item for item in self.kg.G['user'][self.user_id]['interact'] if item in set_cand_items]
        rej_items = [x - self.user_length for x in self.rej_item[-topk:]]##
        # item_friend = {}
        # ##item hypergraph
        # like view #item hypergraph
        lens.append(len(history_items))
        for item in history_items:
            ...
            index.append([item_idx, idx_e])
            value.append(1)
            for fea in self.kg.G['item'][item]['belong_to']:
                feature_id = fea + self.user_length + self.item_length
                feature_idx, idx_v = self.get_key(idx_v_dict, feature_id, idx_v)
                index.append([feature_idx, idx_e])
                value.append(1 / len(self.kg.G['item'][item]['belong_to']))
            idx_e += 1
        # # dislike view #item hypergraph
        lens.append(len(rej_items))
        for item in rej_items:
            ...
            index.append([item_idx, idx_e])
            value.append(1)
            neg_features = set(self.kg.G['item'][item]['belong_to'])-set(self.user_acc_feature)-set(self.reachable_feature)
            for fea in neg_features:
                feature_id = fea + self.user_length + self.item_length
                feature_idx, idx_v = self.get_key(idx_v_dict, feature_id, idx_v)
                index.append([feature_idx, idx_e])
                value.append(1 / len(neg_features))
            idx_e += 1
        # # social view #item hypergraph
        lens.append(len(item_friend))
        for item in item_friend:
            item_id = item + self.user_length
            item_idx, idx_v = self.get_key(idx_v_dict, item_id, idx_v)
            index.append([item_idx, idx_e])
            value.append(1)
            for fea in self.kg.G['item'][item]['belong_to']:
                ...
                index.append([feature_idx, idx_e])
                value.append(1 / len(self.kg.G['item'][item]['belong_to']))
            for friend in item_friend_dict[item]:
                friend_idx, idx_v = self.get_key(idx_v_dict, friend, idx_v)
                index.append([friend_idx, idx_e])
                value.append(1 / len(item_friend_dict[item]))
            idx_e += 1

        # ##user hypergraph
        rej_item_friend_dict = {}
        rej_item_friend_count_dict = {}
        rej_item_friend_feature_dict = {}
        for item in self.rej_item:
            item -= self.user_length##
            for user in self.kg.G['item'][item]['interact']:
                if user in self.kg.G['user'][self.user_id]['friends'] or \
                        len(set(self.kg.G['user'][self.user_id]['interact'])&set(self.kg.G['user'][user]['interact'])) != 0:
                    continue
                if user not in rej_item_friend_count_dict.keys():
                    rej_item_friend_count_dict[user] = 0
                    rej_item_friend_dict[user] = []
                    rej_item_friend_feature_dict[user] = []
                rej_item_friend_count_dict[user] += 1
                rej_item_friend_dict[user].append(item)
                feature_list = [feature for feature in self.kg.G['item'][item]['belong_to'] if feature not in set_reachable_feature and feature not in self.user_acc_feature]
                rej_item_friend_feature_dict[user] = list(set(rej_item_friend_feature_dict[user])|set(feature_list))
        rej_item_friend_count_list = sorted(rej_item_friend_count_dict.items(), key=lambda x: x[1], reverse=True)[:topk]
        rej_item_friend = [x for (x, y) in rej_item_friend_count_list]

        # like view #user hypergraph
        lens.append(1)
        user_idx, idx_v = self.get_key(idx_v_dict, self.user_id, idx_v)
        index.append([user_idx, idx_e])
        value.append(1)
        for item in history_items:
            ...
            item_idx, idx_v = self.get_key(idx_v_dict, item_id, idx_v)
            index.append([item_idx, idx_e])
            value.append(1/len(history_items))
        for fea in self.user_acc_feature:
            fea_id = fea + self.user_length + self.item_length
            feature_idx, idx_v = self.get_key(idx_v_dict, fea_id, idx_v)
            index.append([feature_idx, idx_e])
            value.append(1/len(self.user_acc_feature))
        idx_e += 1
        # # # dislike view #user hypergraph
        lens.append(len(rej_item_friend))
        for user in rej_item_friend:
            user_idx, idx_v = self.get_key(idx_v_dict, user, idx_v)
            index.append([user_idx, idx_e])
            value.append(1)
            for item in rej_item_friend_dict[user]:
                item_id = item + self.user_length
                item_idx, idx_v = self.get_key(idx_v_dict, item_id, idx_v)
                index.append([item_idx, idx_e])
                value.append(1/len(rej_item_friend_dict[user]))
            for fea in rej_item_friend_feature_dict[user]:
                fea_id = fea + self.user_length + self.item_length
                feature_idx, idx_v = self.get_key(idx_v_dict, fea_id, idx_v)
                index.append([feature_idx, idx_e])
                value.append(1 / len(rej_item_friend_feature_dict[user]))
            idx_e += 1

        # # # social view #user hypergraph
        friend_count_list = sorted(friend_item_dict.items(), key=lambda x: len(x[1]), reverse=True)[:topk]
        user_friend = [x for (x, y) in friend_count_list]

        lens.append(len(user_friend))
        for user in user_friend:
            user_idx, idx_v = self.get_key(idx_v_dict, user, idx_v)
            index.append([user_idx, idx_e])
            value.append(1)
            for item in friend_item_dict[user]:
                item_id = item + self.user_length
                item_idx, idx_v = self.get_key(idx_v_dict, item_id, idx_v)
                index.append([item_idx, idx_e])
                value.append(1 / len(friend_item_dict[user]))
            for fea in friend_feature_dict[user]:
                fea_id = fea + self.user_length + self.item_length
                feature_idx, idx_v = self.get_key(idx_v_dict, fea_id, idx_v)
                index.append([feature_idx, idx_e])
                value.append(1 / len(friend_feature_dict[user]))
            idx_e += 1

        hyperneighbors = torch.LongTensor(list(idx_v_dict.keys()))
        index = torch.LongTensor(index)
        value = torch.FloatTensor(value)
        H = torch.sparse.FloatTensor(index.t(), value,
                                          torch.Size([len(hyperneighbors), idx_e]))
        HT = HT_from_H_sp(H.to_dense(), None, out_node=idx_e)



        state = {'lens': copy.deepcopy(lens),
                 'hyperneigh': copy.deepcopy(hyperneighbors),
                 'hyperHT': copy.deepcopy(HT)}
        return state
    def get_key(self, idx_v_dict, key, idx_v):
        if key not in idx_v_dict.keys():
            idx_v_dict[key] = idx_v
            idx_v = idx_v + 1
        return idx_v_dict[key], idx_v
    def step(self, action, sorted_actions, embed=None):
        ...
        if self.cur_conver_step == self.max_turn:
            ...
            ####
            self.ask_times += 1
            if self.test_num > self.cover_num/2:
                for att in asked_feature:
                    if att not in self.ask_att_user_b.keys():
                        self.ask_att_user_b[att] = 0
                    self.ask_att_user_b[att] += 1
            else:
                for att in asked_feature:
                    if att not in self.ask_att_user_a.keys():
                        self.ask_att_user_a[att] = 0
                    self.ask_att_user_a[att] += 1
            ...
            if self.test_num > self.cover_num/2:
                for item in recom_items:
                    if item not in self.rec_item_user_b.keys():
                        self.rec_item_user_b[item] = 0
                    self.rec_item_user_b[item] += 1
            else:
                for item in recom_items:
                    if item not in self.rec_item_user_a.keys():
                        self.rec_item_user_a[item] = 0
                    self.rec_item_user_a[item] += 1
