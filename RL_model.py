##weight_entropy ssl_store_true top3_test
import math
import random
import numpy as np
import os
...
def train(args, kg, dataset, filename):
    ...
    for train_step in range(1, args.max_steps+1):
        ...
        for i_episode in tqdm(range(args.sample_times),desc='sampling'+str(train_step)):
            blockPrint()
            ...
            for t in count():   # user  dialog
                ...
                if args.ssl and i_episode%(3+int(0.1*train_step))==0:
                    sslloss = agent.optimize_ssl(args.batch_size, args.gamma)
        ...
        SR = SR15 / args.sample_times
        AT = AvgT / args.sample_times
        min_AT = max(best_ATs)
        if SR > check or (train_step >10 and AT < min_AT):
            best_ATs.remove(min_AT)
            best_ATs.append(AT)
...
def main():
    ...
    parser.add_argument('--data_name', type=str, default=LAST_FM_STAR, choices=[ LAST_FM_STAR, YELP_STAR,MOVIE, BOOK],
                        help='One of { LAST_FM_STAR, YELP_STAR, BOOK,MOVIE}.')
    parser.add_argument('--entropy_method', type=str, default='entropy', help='entropy_method is one of {entropy, weight entropy,match}')
    ...
    parser.add_argument('--choice_num', type=int, default=2, help='the choice_num')
    ...
    parser.add_argument('--ssl', action='store_false', help='use CL')
    ...
    print('choice_num:{}'.format(args.choice_num))
