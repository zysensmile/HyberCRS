...
def evaluate(args, kg, dataset, filename):
    test_env = EnvDict[args.data_name](kg, dataset, args.data_name, args.embed, seed=args.seed, max_turn=args.max_turn,
                                       cand_num=args.cand_num, cand_item_num=args.cand_item_num, attr_num=args.attr_num, mode='test2', ask_num=args.ask_num, entropy_way=args.entropy_method,
                                       fm_epoch=args.fm_epoch, choice_num=args.choice_num)####
...
def main():
    ...
    data_name = YELP_STAR
    parser.add_argument('--data_name', type=str, default=data_name, choices=[ MOVIE,LAST_FM_STAR, YELP_STAR,BOOK],
    ...
    parser.add_argument('--load_rl_epoch', type=int, default=EpochDict[data_name], help='the epoch of loading RL model')
