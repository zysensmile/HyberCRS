...
class Agent(object):
    ...
    def select_action(self, state, cand1, action_space, is_test=False, is_last_turn=False):
        ...
        if is_test or sample > eps_threshold:
            if is_test and (len(action_space[1]) <= 20 or is_last_turn):
                return torch.tensor(action_space[1][0], device=self.device, dtype=torch.long), action_space[
                    1], state_emb
            with torch.no_grad():
    ...
    def optimize_model(self, BATCH_SIZE, GAMMA):
        ...
        # Double DQN
        # index=self.policy_net(next_state_emb_batch,next_cand_emb_batch,Op=True).argmax(dim=1).view(len(n_states)).to(self.device)
        best_actions = torch.gather(input=next_cand_batch, dim=1,
                                    index=self.policy_net(next_state_emb_batch, next_cand_emb_batch, Op=True).argmax(
                                        dim=1).view(len(n_states), 1).to(self.device))
        # best_actions_emb=next_cand_emb_batch[list(range(len(n_states))), index].view(len(n_states),1,-1)
    ...
    def optimize_ssl(self, BATCH_SIZE, GAMMA):
        if len(self.memory) < BATCH_SIZE:
            return
        ...
        ssl_loss = self.gcn_net.ssl(list(batch.state))
        self.optimizer.zero_grad()
        # ssl_loss.backward()
        for param in self.policy_net.parameters():
            param.grad.data.clamp_(-1, 1)
        self.optimizer.step()
        return ssl_loss.data
        # return 0
